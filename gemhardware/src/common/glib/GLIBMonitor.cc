...
gem::hw::glib::GLIBMonitor::GLIBMonitor(std::shared_ptr<HwGLIB> glib,
                                        GLIBManager *glibManager,
                                        int const &index)
    : GEMMonitor(glibManager->getApplicationLogger(),
                 static_cast<xdaq::Application *>(glibManager), index),
      p_glib(glib) {
  updateMonitorables();
}
void gem::hw::glib::GLIBMonitor::setupHwMonitoring() {
  ...
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("CONTROL", "DAQ.CONTROL"), GEMUpdateType::HW32,
                 "hex");
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("STATUS", "DAQ.STATUS"), GEMUpdateType::HW32,
                 "hex");
  addMonitorable(
      "DAQ Status", "HWMonitoring",
  ...
  addMonitorable(
      ...
      GEMUpdateType::HW32, "hex");
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("L1A_FIFO_NEAR_FULL_CNT",
                                "DAQ.EXT_STATUS.L1A_FIFO_NEAR_FULL_CNT"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("DAQ_FIFO_NEAR_FULL_CNT",
                                "DAQ.EXT_STATUS.DAQ_FIFO_NEAR_FULL_CNT"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("DAQ_ALMOST_FULL_CNT",
                                "DAQ.EXT_STATUS.DAQ_ALMOST_FULL_CNT"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("DAQ Status", "HWMonitoring",
                 std::make_pair("DAQ_ALMOST_FULL_CNT",
                                "DAQ.EXT_STATUS.DAQ_ALMOST_FULL_CNT"),
  ...
  addMonitorable(
      ...
      GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("TTC_PM_PHASE_JUMP_CNT",
                                "TTC.STATUS.CLK.TTC_PM_PHASE_JUMP_CNT"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("TTC_PM_PHASE_JUMP_SIZE",
                                "TTC.STATUS.CLK.TTC_PM_PHASE_JUMP_SIZE"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("TTC_PM_PHASE_JUMP_TIME",
                                "TTC.STATUS.CLK.TTC_PM_PHASE_JUMP_TIME"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
  ...
  addMonitorable(
      ...
      GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("GTH_PM_PHASE_JUMP_CNT",
                                "TTC.STATUS.CLK.GTH_PM_PHASE_JUMP_CNT"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("GTH_PM_PHASE_JUMP_SIZE",
                                "TTC.STATUS.CLK.GTH_PM_PHASE_JUMP_SIZE"),
                 GEMUpdateType::HW32, "hex");
  addMonitorable("TTC", "HWMonitoring",
                 std::make_pair("GTH_PM_PHASE_JUMP_TIME",
                                "TTC.STATUS.CLK.GTH_PM_PHASE_JUMP_TIME"),
                 GEMUpdateType::HW32, "hex");
  ...
  for (uint8_t oh = 0; oh < p_glib->getSupportedOptoHybrids(); ++oh) {
    ...
    addMonitorableSet("DAQ Status", "HWMonitoring");
    addMonitorable("DAQ Status", "HWMonitoring",
                   std::make_pair(ohname.str() + "_STATUS",
                                  "DAQ." + ohname.str() + ".STATUS"),
                   GEMUpdateType::HW32, "hex");
    addMonitorable("DAQ Status", "HWMonitoring",
                   std::make_pair(ohname.str() + "_CORRUPT_VFAT_BLK_CNT",
                                  "DAQ." + ohname.str() +
                                      ".COUNTERS.CORRUPT_VFAT_BLK_CNT"),
                   GEMUpdateType::HW32, "dec");
    addMonitorable("DAQ Status", "HWMonitoring",
                   std::make_pair(ohname.str() + "_EVN",
                                  "DAQ." + ohname.str() + ".COUNTERS.EVN"),
                   GEMUpdateType::HW32, "dec");
    addMonitorable(
        "DAQ Status", "HWMonitoring",
        std::make_pair(ohname.str() + "_EOE_TIMEOUT",
                       "DAQ." + ohname.str() + ".CONTROL.EOE_TIMEOUT"),
        GEMUpdateType::HW32, "hex");
    addMonitorable(
        "DAQ Status", "HWMonitoring",
        std::make_pair(ohname.str() + "_MAX_EOE_TIMER",
                       "DAQ." + ohname.str() + ".COUNTERS.MAX_EOE_TIMER"),
        GEMUpdateType::HW32, "hex");
    addMonitorable(
        "DAQ Status", "HWMonitoring",
        std::make_pair(ohname.str() + "_LAST_EOE_TIMER",
                       "DAQ." + ohname.str() + ".COUNTERS.LAST_EOE_TIMER"),
        GEMUpdateType::HW32, "hex");
    addMonitorable("Trigger Status", "HWMonitoring",
                   std::make_pair(ohname.str() + "_TRIGGER_RATE",
                                  "TRIGGER." + ohname.str() + ".TRIGGER_RATE"),
                   GEMUpdateType::HW32, "dec");
    addMonitorable("Trigger Status", "HWMonitoring",
                   std::make_pair(ohname.str() + "_TRIGGER_CNT",
                                  "TRIGGER." + ohname.str() + ".TRIGGER_CNT"),
                   GEMUpdateType::HW32, "hex");
    for (int cluster = 0; cluster < 8; ++cluster) {
      std::stringstream cluname;
      cluname << "CLUSTER_SIZE_" << cluster;
      addMonitorable(
          "Trigger Status", "HWMonitoring",
          std::make_pair(ohname.str() + "_" + cluname.str() + "_RATE",
                         "TRIGGER." + ohname.str() + "." + cluname.str() +
                             "_RATE"),
          GEMUpdateType::HW32, "dec");
      addMonitorable("Trigger Status", "HWMonitoring",
                     std::make_pair(ohname.str() + "_" + cluname.str() + "_CNT",
                                    "TRIGGER." + ohname.str() + "." +
                                        cluname.str() + "_CNT"),
                     GEMUpdateType::HW32, "hex");
      cluname.str("");
      ...
      addMonitorable(
          "Trigger Status", "HWMonitoring",
          std::make_pair(ohname.str() + "_" + cluname.str(),
                         "TRIGGER." + ohname.str() + "." + cluname.str()),
          GEMUpdateType::HW32, "hex");
    }
  }
...
void gem::hw::glib::GLIBMonitor::updateMonitorables() {
  ...
  for (auto monlist = m_monitorableSetsMap.begin();
    ...
    CMSGEMOS_DEBUG("GLIBMonitor: Updating monitorables in set "
                   << monlist->first);
    for (auto monitem = monlist->second.begin();
      ...
      if (monitem->second.updatetype == GEMUpdateType::HW8) {
        (monitem->second.infoSpace)
            ->setUInt32(monitem->first, p_glib->readReg(address, mask));
  ...
  *out << "<div class=\"xdaq-tab-wrapper\">" << std::endl;
  for (auto monset = monsets.begin(); monset != monsets.end(); ++monset) {
    if ((*monset).rfind("DAQ Status") != std::string::npos) {
      ...
      CMSGEMOS_DEBUG("GLIBMonitor::buildMonitorPage building table "
                     << *monset);
      *out << "<div class=\"xdaq-tab\" title=\"" << *monset << "\" >"
           << std::endl << "<table class=\"xdaq-table\" id=\"" << *monset
           << "_table\">" << std::endl << cgicc::thead() << std::endl
           << cgicc::tr() << std::endl << cgicc::th() << "Register name"
           << cgicc::th() << std::endl << cgicc::th() << "Value" << cgicc::th()
           << std::endl << cgicc::th() << "Register address" << cgicc::th()
           << std::endl << cgicc::th() << "Description" << cgicc::th()
           << std::endl << cgicc::tr() << std::endl << cgicc::thead()
           << std::endl << "<tbody>" << std::endl;
      for (auto monitem = m_monitorableSetsMap.find(*monset)->second.begin();
        ...
        *out << "<td>" << std::endl << monitem->first << "</td>" << std::endl;
        CMSGEMOS_DEBUG(
            "GLIBMonitor::" << monitem->first << " formatted to "
                            << (monitem->second.infoSpace)->getFormattedItem(
                                   monitem->first, monitem->second.format));
        *out << "<td id=\"" << monitem->second.infoSpace->name() << "-"
             << monitem->first << "\">" << std::endl
             << (monitem->second.infoSpace)
                    ->getFormattedItem(monitem->first, monitem->second.format)
             << "</td>" << std::endl;
      ...
      *out << "</tbody>" << std::endl << "</table>" << std::endl << "</div>"
           << std::endl;
    }
  }
  *out << "</div>" << std::endl;
...
void gem::hw::glib::GLIBMonitor::buildDAQStatusTable(xgi::Output *out) {
  ...
  auto monsets = m_infoSpaceMonitorableSetMap.find("HWMonitoring")->second;
  if (std::find(monsets.begin(), monsets.end(), "DAQ Status") ==
      monsets.end()) {
    CMSGEMOS_WARN("Unable to find item set 'DAQ Status' in list of "
  ...
  *out << "<div class=\"xdaq-tab\" title=\"DAQ Status\">" << std::endl
  ...
  CMSGEMOS_DEBUG(
      "GLIBMonitor::buildDAQStatusTable building Common DAQ Status table");
  *out << "<div class=\"xdaq-tab\" title=\""
       << "Common DAQ Status"
       << "\">" << std::endl
       << "<table class=\"xdaq-table\" id=\"CommonDAQStatus_table\">"
       ...
       << "Description" << cgicc::th() << std::endl << cgicc::tr() << std::endl
       << cgicc::thead() << std::endl << "<tbody>" << std::endl;
  for (auto monpair = monset.begin(); monpair != monset.end(); ++monpair) {
    if (monpair->first.find("OH") == std::string::npos) {
      *out << "<tr>" << std::endl;
      *out << "<td>" << std::endl << monpair->first << "</td>" << std::endl;
      CMSGEMOS_INFO(
          "GLIBMonitor::" << monpair->first << " formatted to "
                          << (monpair->second.infoSpace)->getFormattedItem(
                                 monpair->first, monpair->second.format));
      *out << "<td id=\"" << monpair->second.infoSpace->name() << "-"
           << monpair->first << "\">" << std::endl
           << (monpair->second.infoSpace)
                  ->getFormattedItem(monpair->first, monpair->second.format)
           << "</td>" << std::endl;
      *out << "<td>" << std::endl << monpair->second.regname << "</td>"
           << std::endl;
      *out << "<td>" << std::endl << "description"
           << "</td>" << std::endl;
      *out << "</tr>" << std::endl;
  ...
  *out << "</div>" << std::endl;
  CMSGEMOS_INFO(
      "GLIBMonitor::buildDAQStatusTable building Per-link DAQ Status table");
  *out << "<div class=\"xdaq-tab\" title=\""
       << "Per-link DAQ Status"
       << "\" >" << std::endl
       << "<table class=\"xdaq-table\" id=\"Per-linkDAQStatus_table\">"
       << std::endl << cgicc::thead() << std::endl << cgicc::tr() << std::endl
       << cgicc::th() << "Register name" << cgicc::th() << std::endl;
  for (int i = 0; i < 12; ++i)
    *out << cgicc::th() << "Link " << std::setw(2) << std::setfill(' ') << i
         << cgicc::th() << std::endl;
  *out << cgicc::th() << "Register address" << cgicc::th() << std::endl
       << cgicc::th() << "Description" << cgicc::th() << std::endl
       << cgicc::tr() << std::endl << cgicc::thead() << std::endl << "<tbody>"
       << std::endl;
  for (auto monpair = monset.begin(); monpair != monset.end(); ++monpair) {
    if (monpair->first.find("OH0_STATUS") != std::string::npos) {
      CMSGEMOS_INFO("GLIBMonitor::buildDAQStatusTable "
                    << monpair->first << " found, building per-link structure");
      std::array<std::string, 6> linkarray = {
        { "STATUS",        "EVN",            "EOE_TIMEOUT",
          "MAX_EOE_TIMER", "LAST_EOE_TIMER", "CORRUPT_VFAT_BLK_CNT" }
      };
      for (auto regname = linkarray.begin(); regname != linkarray.end();
           ++regname) {
        *out << "<tr>" << std::endl << "<td>" << std::endl << *regname
             << "</td>" << std::endl;
        for (int i = 0; i < 12; ++i) {
          CMSGEMOS_INFO("GLIBMonitor::buildDAQStatusTable creating OH"
                        << i << " table header");
          std::stringstream substr;
          substr << "OH" << i;
          *out << "<td id=\"" << monpair->second.infoSpace->name() << "-OH" << i
               << "_" << *regname << "\">" << std::endl << "N/A"
               << "</td>" << std::endl;
        }
        *out << "<td>" << std::endl << "add"
             << "</td>" << std::endl << "<td>" << std::endl << "desc"
             << "</td>" << std::endl << "</tr>" << std::endl;
      }
      break;
    }
  }
  *out << "</tbody>" << std::endl << "</table>" << std::endl << "</div>"
       << std::endl << "</div>" << std::endl << "</div>" << std::endl;
}
void gem::hw::glib::GLIBMonitor::buildTriggerStatusTable(xgi::Output *out) {
  ...
  auto monsets = m_infoSpaceMonitorableSetMap.find("HWMonitoring")->second;
  if (std::find(monsets.begin(), monsets.end(), "Trigger Status") ==
      monsets.end()) {
    CMSGEMOS_WARN("Unable to find item set 'Trigger Status' in list of "
                  "HWMonitoring monitor sets");
    return;
  }
  auto monset = m_monitorableSetsMap.find("Trigger Status")->second;
  CMSGEMOS_INFO(
      "GLIBMonitor::buildTriggerStatusTable building Trigger Status table");
  *out << "<div class=\"xdaq-tab\" title=\""
       << "Trigger Status"
       << "\" >" << std::endl
       << "<table class=\"xdaq-table\" id=\"TriggerStatus_table\">" << std::endl
       << cgicc::thead() << std::endl << cgicc::tr() << std::endl << cgicc::th()
       << "Register name" << cgicc::th() << std::endl;
  for (int i = 0; i < 12; ++i)
    *out << cgicc::th() << "Link " << std::setw(2) << std::setfill(' ') << i
         << cgicc::th() << std::endl;
  *out << cgicc::th() << "Register address" << cgicc::th() << std::endl
  ...
  for (auto monpair = monset.begin(); monpair != monset.end(); ++monpair) {
    if (monpair->first.find("OH0_TRIGGER_RATE") != std::string::npos) {
      std::array<std::string, 2> linkarray = { { "TRIGGER_RATE",
                                                 "TRIGGER_CNT" } };
      for (auto regname = linkarray.begin(); regname != linkarray.end();
           ++regname) {
        CMSGEMOS_INFO("GLIBMonitor::buildTriggerStatusTable "
                      << monpair->first
                      << " found, building per-link structure");
        *out << "<tr>" << std::endl << "<td>" << std::endl << *regname
             << "</td>" << std::endl;
        for (int i = 0; i < 12; ++i) {
          CMSGEMOS_INFO("GLIBMonitor::buildTriggerStatusTable creating OH"
                        << i << " table header");
          *out << "<td id=\"" << monpair->second.infoSpace->name() << "-OH" << i
               << "_" << *regname << "\">" << std::endl << "N/A"
               << "</td>" << std::endl;
      ...
      for (int j = 0; j < 8; ++j) {
        ...
        *out << "<tr>" << std::endl << "<td>" << std::endl << specregname.str()
             << "</td>" << std::endl;
        for (int i = 0; i < 12; ++i) {
          CMSGEMOS_INFO("GLIBMonitor::buildTriggerStatusTable creating OH"
                        << i << " table header");
          *out << "<td id=\"" << monpair->second.infoSpace->name() << "-OH" << i
               << "_" << specregname.str() << "\">" << std::endl << "N/A"
               << "</td>" << std::endl;
        }
        *out << "<td>" << std::endl << "add"
             << "</td>" << std::endl << "<td>" << std::endl << "desc"
...
void gem::hw::glib::GLIBMonitor::reset() {
  ...
  for (auto infoSpace = m_infoSpaceMap.begin();
    ...
    CMSGEMOS_DEBUG("GLIBMonitor::reset removing " << infoSpace->first
                                                  << " from p_timer");
    try {
      p_timer->remove(infoSpace->first);
    }
    catch (toolbox::task::exception::Exception &te) {
      CMSGEMOS_ERROR("GLIBMonitor::Caught exception while removing timer task "
  ...
  try {
    ...
    CMSGEMOS_ERROR("GLIBMonitor::Caught exception while removing timer "
                   << m_timerName << " " << te.what());
  }
