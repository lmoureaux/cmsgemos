...
gem::hw::GEMHwDevice::GEMHwDevice(std::string const &deviceName,
    ...
    : xhal::XHALInterface(deviceName),
      uhal::HwInterface(std::shared_ptr<uhal::ConnectionManager>(
          new uhal::ConnectionManager("file://${GEM_ADDRESS_TABLE_PATH}/" +
                                      connectionFile))->getDevice(deviceName)),
...
gem::hw::GEMHwDevice::GEMHwDevice(std::string const &deviceName,
    ...
    : xhal::XHALInterface(deviceName),
      uhal::HwInterface(uhal::ConnectionManager::getDevice(
          deviceName, connectionURI, addressTable)),
      m_gemLogger(log4cplus::Logger::getInstance(deviceName)),
      m_hwLock(toolbox::BSem::FULL, true) {
  CMSGEMOS_DEBUG("GEMHwDevice::ctor done");
}
gem::hw::GEMHwDevice::GEMHwDevice(std::string const &deviceName,
    ...
    : xhal::XHALInterface(deviceName), uhal::HwInterface(uhalDevice),
      m_gemLogger(log4cplus::Logger::getInstance(deviceName)),
      m_hwLock(toolbox::BSem::FULL, true) {
...
uint32_t gem::hw::GEMHwDevice::readReg(std::string const &name) {
  ...
  CMSGEMOS_DEBUG(
      "GEMHwDevice::readReg "
      << name << std::endl << "Path  " << this->getNode(name).getPath()
      << std::endl << "Address 0x" << std::hex
      << this->getNode(name).getAddress() << std::dec << std::endl << "Mask 0x"
      << std::hex << this->getNode(name).getMask() << std::dec << std::endl
      << "Permission " << this->getNode(name).getPermission() << std::endl
      << "Mode " << this->getNode(name).getMode() << std::endl << "Size "
      << this->getNode(name).getSize() << std::endl << std::endl);
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      this->dispatch();
      res = val.value();
      CMSGEMOS_TRACE("GEMHwDevice::Successfully read register "
                     << name.c_str() << " with value 0x" << std::setfill('0')
                     << std::setw(8) << std::hex << res << std::dec
                     << " retry count is " << retryCount
                     << ". Should move on to next operation");
      return res;
    }
    catch (uhal::exception::exception const &err) {
      std::string msgBase = toolbox::toString(
          "Could not read register '%s' (uHAL)", name.c_str());
      std::string msg =
      ...
      if (knownErrorCode(errCode)) {
        ...
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to read register "
                         << name << ". retryCount(" << retryCount << ")"
                         << std::endl);
      ...
      std::string msgBase =
          toolbox::toString("Could not read register '%s' (std)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to read register %s",
      name.c_str());
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
  return res;
}
uint32_t gem::hw::GEMHwDevice::readReg(uint32_t const &address) {
  ...
  unsigned retryCount = 0;
  uint32_t res = 0x0;
  CMSGEMOS_TRACE("GEMHwDevice::gem::hw::GEMHwDevice::readReg 0x"
                 << std::setfill('0') << std::setw(8) << std::hex << address
                 << std::dec << std::endl);
  while (retryCount < MAX_IPBUS_RETRIES) {
    ++retryCount;
    try {
      ...
      this->dispatch();
      res = val.value();
      CMSGEMOS_TRACE("GEMHwDevice::Successfully read register 0x"
                     << std::setfill('0') << std::setw(8) << std::hex << address
                     << std::dec << " with value 0x" << std::setfill('0')
                     << std::setw(8) << std::hex << res << std::dec
                     << " retry count is " << retryCount
                     << ". Should move on to next operation");
      return res;
      ...
      std::string msgBase =
          toolbox::toString("Could not read register '0x%08x' (uHAL)", address);
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        ++retryCount;
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to read register 0x"
                         << std::setfill('0') << std::setw(8) << std::hex
                         << address << std::dec << ". retryCount(" << retryCount
                         << ")" << std::endl);
        updateErrorCounters(errCode);
        continue;
      } else {
      ...
      std::string msgBase =
          toolbox::toString("Could not read register '0x%08x' (std)", address);
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to read register 0x%08x",
      address);
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
  return res;
}
uint32_t gem::hw::GEMHwDevice::readReg(uint32_t const &address,
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      CMSGEMOS_TRACE("GEMHwDevice::Successfully read register 0x"
                     << std::setfill('0') << std::setw(8) << std::hex << address
                     << std::dec << " with mask " << std::hex << mask
                     << std::dec << " with value " << std::setfill('0')
                     << std::setw(8) << std::hex << res << std::dec
                     << " retry count is " << retryCount
                     << ". Should move on to next operation");
      return res;
      ...
      if (knownErrorCode(errCode)) {
        ...
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to read register 0x"
                         << std::setfill('0') << std::setw(8) << std::hex
                         << address << std::dec << " with mask " << std::hex
                         << address << std::dec << ". retryCount(" << retryCount
                         << ")" << std::endl);
        updateErrorCounters(errCode);
        continue;
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      for (auto curReg = regList.begin(); curReg != regList.end(); ++curReg) {
        vals.push_back(
            std::make_pair(curReg->first, this->getNode(curReg->first).read()));
        ++counter;
      ...
      CMSGEMOS_DEBUG("GEMHwDevice::readRegs dispatched "
                     << dispatchcounter << " calls for " << counter
                     << " registers");
      ...
      std::string msg =
          toolbox::toString("%s (uHAL): %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        updateErrorCounters(errCode);
        continue;
      ...
      for (auto curReg = regList.begin(); curReg != regList.end(); ++curReg)
        msgBase += toolbox::toString(" '%s'", curReg->first.c_str());
      std::string msg =
          toolbox::toString("%s (std): %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  }
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to read registers");
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
}
void gem::hw::GEMHwDevice::readRegs(addressed_register_pair_list &regList,
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      int counter{ 0 }, dispatchcounter{ 0 };
      for (auto curReg = regList.begin(); curReg != regList.end(); ++curReg) {
        vals.push_back(std::make_pair(curReg->first,
                                      this->getClient().read(curReg->first)));
        ++counter;
        if (freq > 0 && counter % freq == 0) {
          this->dispatch();
          ++dispatchcounter;
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      std::vector<std::pair<std::pair<uint32_t, uint32_t>,
                            uhal::ValWord<uint32_t> > > vals;
      int counter{ 0 }, dispatchcounter{ 0 };
      for (auto curReg = regList.begin(); curReg != regList.end(); ++curReg) {
        vals.push_back(std::make_pair(
            std::make_pair(curReg->first.first, curReg->first.second),
      ...
      std::string msgBase = "Could not read from register in list:";
      for (auto curReg = regList.begin(); curReg != regList.end(); ++curReg)
        msgBase += toolbox::toString(" '0x%08x mask 0x%08x'",
                                     curReg->first.first, curReg->first.second);
      std::string msg =
          toolbox::toString("%s (uHAL): %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
...
void gem::hw::GEMHwDevice::writeReg(std::string const &name,
  ...
  gem::utils::LockGuard<gem::utils::Lock> guardedLock(m_hwLock);
  unsigned retryCount = 0;
  CMSGEMOS_DEBUG("GEMHwDevice::writeReg "
                 << name << std::endl << "Path  "
                 << this->getNode(name).getPath() << std::endl << "Address 0x"
                 << std::hex << this->getNode(name).getAddress() << std::dec
                 << std::endl << "Mask 0x" << std::hex
                 << this->getNode(name).getMask() << std::dec << std::endl
                 << "Permission " << this->getNode(name).getPermission()
                 << std::endl << "Mode " << this->getNode(name).getMode()
                 << std::endl << "Size " << this->getNode(name).getSize()
                 << std::endl << std::endl);
  while (retryCount < MAX_IPBUS_RETRIES) {
    ++retryCount;
    try {
      ...
      if (this->getNode(name).getPermission() != uhal::defs::WRITE)
        if (rval.value() != val) {
          std::string msgBase =
              toolbox::toString("WriteValueMismatch write (0x%x) to register "
                                "'%s' resulted in 0x%x (uHAL)",
                                val, name.c_str(), rval.value());
          XCEPT_RAISE(gem::hw::exception::WriteValueMismatch,
                      toolbox::toString("%s.", msgBase.c_str()));
        }
      return;
    }
    catch (uhal::exception::exception const &err) {
      std::string msgBase = toolbox::toString(
          "Could not write to register '%s' (uHAL)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        ++retryCount;
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to write value 0x"
                         << std::hex << val << std::dec << " to register "
                         << name << ". retryCount(" << retryCount << ")"
                         << std::endl);
        updateErrorCounters(errCode);
        continue;
      ...
      std::string msgBase = toolbox::toString(
          "Could not write to register '%s' (std)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to write to register %s",
      name.c_str());
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
}
void gem::hw::GEMHwDevice::writeReg(uint32_t const &address,
                                    uint32_t const val) {
  gem::utils::LockGuard<gem::utils::Lock> guardedLock(m_hwLock);
  unsigned retryCount = 0;
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      std::string msgBase = toolbox::toString(
          "Could not write to register '0x%08x' (uHAL)", address);
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        ...
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to write value 0x"
                         << std::hex << val << std::dec << " to register 0x"
                         << std::setfill('0') << std::setw(8) << std::hex
                         << address << std::dec << ". retryCount(" << retryCount
                         << ")" << std::endl);
        updateErrorCounters(errCode);
      ...
      std::string msgBase = toolbox::toString(
          "Could not write to register '0x%08x' (std)", address);
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to write to register 0x%08x",
      address);
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
}
void gem::hw::GEMHwDevice::writeRegs(register_pair_list const &regList,
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      CMSGEMOS_DEBUG("GEMHwDevice::writeRegs dispatched "
                     << dispatchcounter << " calls for " << counter
                     << " registers");
      return;
    }
...
void
gem::hw::GEMHwDevice::writeValueToRegs(std::vector<std::string> const &regNames,
...
std::vector<uint32_t> gem::hw::GEMHwDevice::readBlock(std::string const &name) {
  ...
  CMSGEMOS_TRACE("GEMHwDevice::reading block " << name << " which has size "
                                               << numWords);
...
std::vector<uint32_t> gem::hw::GEMHwDevice::readBlock(std::string const &name,
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      uhal::ValVector<uint32_t> values =
          this->getNode(name).readBlock(numWords);
      this->dispatch();
      ...
      std::string msgBase =
          toolbox::toString("Could not read block '%s' (uHAL)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        ++retryCount;
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to read block "
                         << name << " with " << numWords << " words"
                         << ". retryCount(" << retryCount << ")" << std::endl
                         << "error was " << errCode << std::endl);
        updateErrorCounters(errCode);
        continue;
      } else {
        CMSGEMOS_ERROR("GEMHwDevice::" << msg);
      ...
      std::string msgBase =
          toolbox::toString("Could not read block '%s' (std)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to read block");
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
  return res;
}
uint32_t gem::hw::GEMHwDevice::readBlock(std::string const &name,
                                         uint32_t *buffer,
                                         size_t const &numWords) {
  return 0;
}
void gem::hw::GEMHwDevice::writeBlock(std::string const &name,
  ...
  while (retryCount < MAX_IPBUS_RETRIES) {
    ...
    try {
      ...
      std::string msgBase = toolbox::toString(
          "Could not write to block '%s' (uHAL)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      std::string errCode = toolbox::toString("%s", err.what());
      if (knownErrorCode(errCode)) {
        ++retryCount;
        if (retryCount > (MAX_IPBUS_RETRIES - 1))
          CMSGEMOS_DEBUG("GEMHwDevice::Failed to write block "
                         << name << ". retryCount(" << retryCount << ")"
                         << std::endl);
        updateErrorCounters(errCode);
        continue;
      ...
      std::string msgBase = toolbox::toString(
          "Could not write to block '%s' (std)", name.c_str());
      std::string msg =
          toolbox::toString("%s: %s.", msgBase.c_str(), err.what());
      CMSGEMOS_ERROR("GEMHwDevice::" << msg);
    }
  ...
  std::string msg = toolbox::toString(
      "Maximum number of retries reached, unable to write block %s",
      name.c_str());
  CMSGEMOS_ERROR("GEMHwDevice::" << msg);
}
...
void gem::hw::GEMHwDevice::writeFIFO(std::string const &name,
                                     std::vector<uint32_t> const values) {
...
void gem::hw::GEMHwDevice::generalReset() { return; }
void gem::hw::GEMHwDevice::counterReset() { return; }
void gem::hw::GEMHwDevice::linkReset(uint8_t const &link) { return; }
