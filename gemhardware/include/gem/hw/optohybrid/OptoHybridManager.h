...
protected:
  ...
  virtual void
  initializeAction() throw(gem::hw::optohybrid::exception::Exception);
  virtual void
  configureAction() throw(gem::hw::optohybrid::exception::Exception);
  ...
  public:
    ...
    inline std::string toString() {
      ...
      os << "Mode      :" << Mode.toString() << std::endl
         ...
         << "Output5Src:" << Output5Src.toString() << std::endl << std::endl;
      return os.str();
    };
  };
  ...
  public:
    ...
    inline std::string toString() {
      ...
      os << "ContReg0   :" << ContReg0.toString() << std::endl
         ...
         << "VThreshold2:" << VThreshold2.toString() << std::endl << std::endl;
      return os.str();
    };
  };
  ...
  public:
    ...
    inline std::string toString() {
      ...
      os << "present:" << present.toString() << std::endl
         ...
         << "vfatBroadcastMask:0x" << std::hex << vfatBroadcastMask.value_
         << std::dec << std::endl << "vfatSBitList:" << vfatSBitList.toString()
         << std::endl << "vfatSBitMask:0x" << std::hex << vfatSBitMask.value_
         << std::dec << std::endl << "vfatSBitList:" << vfatSBitList.toString()
         << std::endl << "vfatSBitMask:0x" << std::hex << vfatSBitMask.value_
         << std::dec << std::endl << "triggerSource:0x" << std::hex
         << triggerSource.value_ << std::dec << std::endl << "refClkSrc:0x"
         << std::hex << refClkSrc.value_ << std::dec << std::endl
         << "sbitConfig" << sbitConfig.bag.toString() << std::endl
         << "commonVFATSettings" << commonVFATSettings.bag.toString()
         << std::endl << std::endl;
      return os.str();
    };
  };
...
private:
  void createOptoHybridInfoSpaceItems(is_toolbox_ptr is_optohybrid,
                                      optohybrid_shared_ptr optohybrid);
  mutable gem::utils::Lock m_deviceLock;
  std::array<std::array<optohybrid_shared_ptr, MAX_OPTOHYBRIDS_PER_AMC>,
             MAX_AMCS_PER_CRATE> m_optohybrids;
  std::array<
      std::array<std::shared_ptr<OptoHybridMonitor>, MAX_OPTOHYBRIDS_PER_AMC>,
      MAX_AMCS_PER_CRATE> m_optohybridMonitors;
  std::array<std::array<is_toolbox_ptr, MAX_OPTOHYBRIDS_PER_AMC>,
             MAX_AMCS_PER_CRATE> is_optohybrids;
  ...
  std::array<std::array<uint32_t, MAX_OPTOHYBRIDS_PER_AMC>, MAX_AMCS_PER_CRATE>
  m_sbitMask;
  std::array<std::array<std::vector<std::pair<uint8_t, uint32_t> >,
                        MAX_OPTOHYBRIDS_PER_AMC>,
             MAX_AMCS_PER_CRATE> m_vfatMapping;
